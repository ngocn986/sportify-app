...
function SearchArtist({ data }) {
  ...
  console.log('artist', artist.artists?.items);
  return (
    <div className='bg-[#101010]'>
      <div className='flex flex-wrap gap-4 px-7'>
        {artist.artists?.items.map((item, i) => (
          <NavLink key={i} to={item.external_urls?.spotify}>
            <div className='mt-4 max-w-[176px] h-[255px] cursor-pointer p-4 rounded-md bg-[#161616] hover:bg-[#242424] duration-500 relative group'>
              ...
              {item.images[0]?.url ? (
                <img
                  className='w-[144px] h-[144px] rounded-full mb-4'
